...
fn decode_node(
    ...
    match node.ty().map(ToString::to_string).as_deref() {
        Some("list") => decode_list(node),

        Some(ty) => Err(SerializationError::unsupported_type(ty)),

        None => decode_standard_node(node),
    }
}

fn decode_standard_node(
    node: &KdlNode,
) -> Result<Value, SerializationError> {
    ...
    match (entries.is_empty(), children) {
        ...
        (true, None) => Err(SerializationError::invalid_structure(
            format!(
        ...
        (false, Some(_)) => Err(SerializationError::invalid_structure(
            format!(
...
fn decode_list(
    ...
    let children = node.children().ok_or_else(|| {
        SerializationError::invalid_structure(
            format!("list `{}` has no children", node.name()),
        )
    })?;
...
fn decode_entry(
    ...
    match entry.ty().map(ToString::to_string).as_deref() {
        Some("ref") => {
            let value = entry
                .value()
                .as_string()
                .ok_or_else(|| {
                    SerializationError::invalid_structure(
                        "a reference must contain a string",
                    )
                })?;
    ...
    match value {
        ...
        KdlValue::Integer(value) => {
            let value = i64::try_from(*value)
                .map_err(|_| SerializationError::IntegerOutOfRange(*value))?;
...
fn encode_document(
    ...
    document.nodes_mut().extend(
        values
            .iter()
            .map(|(name, value)| encode_node(name, value)),
    );
...
fn encode_entry(
    ...
    match value {
        ...
        Value::Struct(_) | Value::List(_) => {
            unreachable!(
                "structured values must be encoded as nodes"
            )
        }
    }
}