...
pub fn page_from_value(
    value: Value,
) -> Result<RawPage, SerializationError> {
    let mut root = into_struct(
        value,
        "page document",
    )?;
...
fn decode_page_data(
    value: Value,
) -> Result<PageData, SerializationError> {
    let mut values = into_struct(
        value,
        "`page`",
    )?;
...
pub fn page_to_value(
    raw_page: &RawPage,
) -> Value {
    let data = raw_page.data();

    let page_value = Value::structure(
        BTreeMap::from([
            (
                "name".into(),
            ...
            (
                "priority".into(),
                Value::integer(
                    i64::from(data.priority()),
                ),
            ),
        ]),
    );

    let entries = raw_page
        .entries()
        .iter()
